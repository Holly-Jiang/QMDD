...
// make a QMDD nonterminal node using existing space pointed to by p
  ...
  if(e.w != 1) {
   ...
   if (debugSift) { printf("Debug: adjusting renormalization factor of node %d. From ", (intptr_t) e.p); Cprint(gCvalue(e.p->renormFactor));}
    ...
    if (debugSift) { printf(" to ");  Cprint(gCvalue(e.p->renormFactor)); printf("\n");}
    e.w=1;
